# Job Matcher AI — TypeScript Firefox Extension — Agents & Components Guide

This document describes the role of each component ("agent" or "actor") within the **Job Matcher AI** TypeScript Firefox extension and how they communicate.

---

## 1. Actor Directory & Roles

### 1.1 The Background Agent (`service-worker.ts`)
...
* **Responsibilities**:
  ...
  - Directly interfaces with the local Ollama LLM (`localhost:11434`).

### 1.2 The UI Agent (`popup.ts`)
...
### 1.3 The Configuration Agent (`options.ts`)
* **Role**: Handles extension settings. It runs in a dedicated options tab (`options.html`).
* **Responsibilities**:
  - Configures Ollama connection (base URL, model) in local storage.
...
```
+------------+                  +------------------+                  +-------------------+
| popup.ts   | --(ANALYZE_JOB)-> | service-worker.ts| --(Injects)----> | Injected Script   |
...
1. **`ANALYZE_JOB`**: Sent by `popup.ts` to trigger matching.
...
3. **`AUDIO_SOLVE`**: Sent by `popup.ts` when asking a voice question.
